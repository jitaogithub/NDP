#ifndef FAT_TREE_3_TO_1_OVERSUBSCRIBED
#define FAT_TREE_3_TO_1_OVERSUBSCRIBED
#include <ostream>
...
#define HOST_POD_SWITCH(src) (src / K)
// #define HOST_POD_ID(src) src%NSRV
#define HOST_POD(src) (src / (NC * 3))

#define MIN_POD_ID(pod_id) (pod_id * K / 3)
#define MAX_POD_ID(pod_id) ((pod_id + 1) * K / 3 - 1)
...
class FatTree3to1Oversubscribed : public Topology {
 public:
  /*
  ...
  */
  ...
  FatTree3to1Oversubscribed(int no_of_nodes, mem_b queuesize, Logfile* log,
                            EventList* ev, FirstFit* f, queue_type q);
  FatTree3to1Oversubscribed(int no_of_nodes, mem_b queuesize, Logfile* log,
                            EventList* ev, FirstFit* f, queue_type q, int fail);
