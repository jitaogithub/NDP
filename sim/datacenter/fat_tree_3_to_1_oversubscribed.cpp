...
FatTree3to1Oversubscribed::FatTree3to1Oversubscribed(int no_of_nodes,
                                                     mem_b queuesize,
...
void FatTree3to1Oversubscribed::set_params(int no_of_nodes) {
...
Queue* FatTree3to1Oversubscribed::alloc_src_queue(QueueLogger* queueLogger) {
  return new PriorityQueue(speedFromMbps((uint64_t)HOST_NIC),
                           memFromPkt(FEEDER_BUFFER), *eventlist, queueLogger);
}

Queue* FatTree3to1Oversubscribed::alloc_queue(QueueLogger* queueLogger,
                                              mem_b queuesize) {
  return alloc_queue(queueLogger, HOST_NIC, queuesize);
}

Queue* FatTree3to1Oversubscribed::alloc_queue(QueueLogger* queueLogger,
                                              uint64_t speed, mem_b queuesize) {
  if (qt == RANDOM)
    ...
    return new CompositeQueue(speedFromMbps(speed), queuesize, *eventlist,
                              queueLogger);
  else if (qt == CTRL_PRIO)
    return new CtrlPrioQueue(speedFromMbps(speed), queuesize, *eventlist,
                             queueLogger);
  else if (qt == ECN)
    return new ECNQueue(speedFromMbps(speed), memFromPkt(2 * SWITCH_BUFFER),
                        *eventlist, queueLogger, memFromPkt(15));
  else if (qt == LOSSLESS)
    return new LosslessQueue(speedFromMbps(speed), memFromPkt(50), *eventlist,
                             queueLogger, NULL);
    ...
    return new LosslessOutputQueue(speedFromMbps(speed), memFromPkt(10000),
                                   *eventlist, queueLogger, 1, memFromPkt(16));
  assert(0);
}

void FatTree3to1Oversubscribed::init_network() {
  ...
  if (qt == LOSSLESS)
    for (int j = 0; j < NK; j++) {
      ...
      if (j < NC) switches_c[j] = new Switch("Switch_Core_" + ntoa(j));
    }
  ...
  for (int j = 0; j < NK; j++) {
    for (int l = 0; l < K; l++) {
      ...
      if (qt == LOSSLESS) {
        ...
        ((LosslessQueue*)queues_nlp_ns[j][k])
            ->setRemoteEndpoint(queues_ns_nlp[k][j]);
  ...
  for (int j = 0; j < NK; j++) {
    int podid = 3 * j / K;
    // Connect the lower layer switch to the upper layer switches in the same
    // pod
    for (int k = MIN_POD_ID(podid); k <= MAX_POD_ID(podid); k++) {
      ...
      if (qt == LOSSLESS) {
        ...
        ((LosslessQueue*)queues_nlp_nup[j][k])
            ->setRemoteEndpoint(queues_nup_nlp[k][j]);
        switches_up[k]->addPort(queues_nup_nlp[k][j]);
        ((LosslessQueue*)queues_nup_nlp[k][j])
            ->setRemoteEndpoint(queues_nlp_nup[j][k]);
      } else if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN) {
  ...
  for (int j = 0; j < NK; j++) {
    ...
    for (int l = 0; l < K / 3; l++) {
      ...
      if ((l + j * K / 2) < failed_links) {
        queues_nc_nup[k][j] =
            alloc_queue(queueLogger, HOST_NIC / 10, _queuesize);
        cout << "Adding link failure for j" << ntoa(j) << " l " << ntoa(l)
             << endl;
      } else
      ...
      if (qt == LOSSLESS) {
        ...
        ((LosslessQueue*)queues_nup_nc[j][k])
            ->setRemoteEndpoint(queues_nc_nup[k][j]);
        switches_c[k]->addPort(queues_nc_nup[k][j]);
        ((LosslessQueue*)queues_nc_nup[k][j])
            ->setRemoteEndpoint(queues_nup_nc[j][k]);
      } else if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN) {
  ...
  if (qt == LOSSLESS)
    for (int j = 0; j < NK; j++) {
      ...
      if (j < NC) switches_c[j]->configureLossless();
    }
}

void check_non_null(Route* rt);

vector<const Route*>* FatTree3to1Oversubscribed::get_paths(int src, int dest) {
  ...
  // Queue* pqueue = new Queue(speedFromMbps((uint64_t)HOST_NIC),
  // memFromPkt(FEEDER_BUFFER), *eventlist, simplequeuelogger);
  ...
  if (HOST_POD_SWITCH(src) == HOST_POD_SWITCH(dest)) {
    ...
    if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
      routeout->push_back(
          queues_ns_nlp[src][HOST_POD_SWITCH(src)]->getRemoteEndpoint());
    ...
    if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
      routeback->push_back(
          queues_ns_nlp[dest][HOST_POD_SWITCH(dest)]->getRemoteEndpoint());
    ...
    for (int upper = MIN_POD_ID(pod); upper <= MAX_POD_ID(pod); upper++) {
      ...
      if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
        routeout->push_back(
            queues_nlp_nup[HOST_POD_SWITCH(src)][upper]->getRemoteEndpoint());
      ...
      if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
        routeout->push_back(
            queues_nup_nlp[upper][HOST_POD_SWITCH(dest)]->getRemoteEndpoint());
      ...
      if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
        routeback->push_back(
            queues_nlp_nup[HOST_POD_SWITCH(dest)][upper]->getRemoteEndpoint());
      ...
      if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
        routeback->push_back(
            queues_nup_nlp[upper][HOST_POD_SWITCH(src)]->getRemoteEndpoint());
    ...
    for (int upper = MIN_POD_ID(pod); upper <= MAX_POD_ID(pod); upper++)
      for (int core = (upper % (K / 3)) * K / 3;
           core < ((upper % (K / 3)) + 1) * K / 3; core++) {
        // upper is nup
        ...
        // 3 * core / K is core group and offset in pod
        int upper2 = HOST_POD(dest) * K / 3 + 3 * core / K;
        ...
        if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
          routeback->push_back(
              queues_nup_nc[upper2][core]->getRemoteEndpoint());
...
void FatTree3to1Oversubscribed::count_queue(Queue* queue) {
...
int FatTree3to1Oversubscribed::find_lp_switch(Queue* queue) {
  // first check ns_nlp
  for (int i = 0; i < NSRV; i++)
    for (int j = 0; j < NK; j++)
      if (queues_ns_nlp[i][j] == queue) return j;
  ...
  for (int i = 0; i < NK; i++)
    for (int j = 0; j < NK; j++)
      if (queues_nup_nlp[i][j] == queue) return j;

  return -1;
}

int FatTree3to1Oversubscribed::find_up_switch(Queue* queue) {
  count_queue(queue);
  // first check nc_nup
  for (int i = 0; i < NC; i++)
    for (int j = 0; j < NK; j++)
      if (queues_nc_nup[i][j] == queue) return j;

  // check nlp_nup
  for (int i = 0; i < NK; i++)
    for (int j = 0; j < NK; j++)
      if (queues_nlp_nup[i][j] == queue) return j;

  return -1;
}

int FatTree3to1Oversubscribed::find_core_switch(Queue* queue) {
  count_queue(queue);
  // first check nup_nc
  for (int i = 0; i < NK; i++)
    for (int j = 0; j < NC; j++)
      if (queues_nup_nc[i][j] == queue) return j;

  return -1;
}

int FatTree3to1Oversubscribed::find_destination(Queue* queue) {
  // first check nlp_ns
  for (int i = 0; i < NK; i++)
    for (int j = 0; j < NSRV; j++)
      if (queues_nlp_ns[i][j] == queue) return j;

  return -1;
}

void FatTree3to1Oversubscribed::print_path(std::ofstream& paths, int src,
                                           const Route* route) {
  paths << "SRC_" << src << " ";
